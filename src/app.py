...
def create_figure_radar(main_df, players, metrics):
    ...
    for player in players:
        ...
        for metric in metrics:
            if main_df[main_df['Player'].isin(players)][metric].max() == 0: # Can't divide by 0. (See what I do in the next else).
                norm_values_list.append(0) 
            else:
                norm_values_list.append(main_df[main_df['Player'] == player][metric].iloc[0] / main_df[main_df['Player'].isin(players)][metric].max()) #Divide by max value in the dataset with the selected players to normalise metric
...
app.layout = html.Div(children=[
    ...
    html.Div(children=[
                    ...
                    html.H1(id='main_header',
                            children=html.Div(['Player comparison ',
                                               html.Span("It reads a database with random values because I can’t publish the actual ones.", style={'color': 'rgb(176, 176, 176)',
                                                                       'font-size':20}),
                                                ]),
                            style={
                                'textAlign': 'left',
    ...
    html.Div(children=['Contact me on ',
                       html.A('LinkedIn', href='https://www.linkedin.com/in/david-fernandez-11a715170/', target="_blank"),
                       '. Find the code on my ',
                       html.A('Github', href='https://github.com/davidferg/player-comparison', target="_blank"),
                       '.'],
